 ...
 */
s32b Term_queue_space(void)
{
	/* Find the gap if the tail is before the head. */
	s32b space = Term->key_tail - Term->key_head;
 ...
 */
static errr Metadpy_update(s32b flush, s32b sync, s32b discard)
{
	/* Flush if desired */
 ...
 */
static errr Infowin_init_data(Window dad, s32b x, s32b y, s32b w, s32b h,
                              s32b b, Pixell fg, Pixell bg)
{
	Window xid;
 ...
 */
static errr Infowin_impell(s32b x, s32b y)
{
	/* Execute the request */
 ...
 * Resize an infowin
 */
static errr Infowin_resize(s32b w, s32b h)
{
	/* Execute the request */
 ...
 */
static errr Infowin_locate(s32b x, s32b y, s32b w, s32b h)
{
	/* Execute the request */
 ...
 */
static s32b Infoclr_Opcode(cptr str)
{
	register s32b i;
 ...
 */
static errr Infoclr_init_data(Pixell fg, Pixell bg, s32b op, s32b stip)
{
	infoclr *iclr = Infoclr;
 ...
 * Standard Text
 */
static errr Infofnt_text_std(s32b x, s32b y, cptr str, s32b len)
{
	s32b i;
 ...
 */
static errr Infofnt_text_non(s32b x, s32b y, cptr str, s32b len)
{
	s32b w, h;
 ...
 */
static void pixel_to_square(s32b * const x, s32b * const y,
                            const s32b ox, const s32b oy)
{
 ...
 */
static void square_to_pixel(s32b * const x, s32b * const y,
                            const s32b ox, const s32b oy)
{
 ...
 */
static void mark_selection_clear(s32b x1, s32b y1, s32b x2, s32b y2)
{
 ...
 */
static void mark_selection_mark(s32b x1, s32b y1, s32b x2, s32b y2)
{
 ...
 */
static void copy_x11_start(s32b x, s32b y)
{
 ...
 */
static void copy_x11_cont(s32b x, s32b y, u32b buttons)
{
 ...
 */
static s32b add_char_string(char *buf, byte a, char c)
{
	*buf = c;
	return 1;
}
 ...
 */
static void handle_button(Time time, s32b x, s32b y, s32b button,
                          bool press)
{
...
static errr CheckEvent(bool wait)
{
	...
	*/
	...
	case ButtonRelease:
		{
			...
			/* Where is the mouse */
			s32b x = xev->xbutton.x;
			s32b y = xev->xbutton.y;
	...
	case MotionNotify:
		{
			s32b x = xev->xmotion.x;
			s32b y = xev->xmotion.y;
			u32b z = xev->xmotion.state;
...
static errr Term_pict_x11(s32b x, s32b y, s32b n, const byte *ap, const char *cp,
 ...
 */
static errr Term_xtra_x11_level(s32b v)
{
	term_data *td = (term_data*)(Term->data);
...
static s32b arg_graphics_type = GRAPHICS_16X16;
static s32b num_term = 1;
static errr Term_xtra_x11(s32b n, s32b v)
{
	...
	/* Load the gfx now that virtual paths are set up */
	case TERM_XTRA_LOAD_GFX:
		{
			...
			if (arg_graphics)
			{
				/* work out what the filename is going to be */
				if (arg_graphics_type == GRAPHICS_8X8)
				{
					name          = "8x8.bmp";
					pict_wid      = 8;
					pict_hgt      = 8;
				...
				else if (arg_graphics_type == GRAPHICS_16X16)
				{
					name          = "16x16.bmp";
					pict_wid      = 16;
					pict_hgt      = 16;
				...
				else if (arg_graphics_type == GRAPHICS_32X32)
				{
					name          = "32x32.bmp";
					pict_wid      = 32;
					pict_hgt      = 32;
				...
				path_build(filename, 1024, TENGINE_DIR_GRAF, name);
				use_graphics = TRUE;
			...
			if (use_graphics)
			{
				...
				for (i = 0; i < num_term; i++)
				{
					term_data *td = &data[i];
					s32b ii, jj;
					s32b depth = DefaultDepth(dpy, DefaultScreen(dpy));
 ...
 */
static errr Term_curs_x11(s32b x, s32b y)
{
	/* Draw the cursor */
	Infoclr_set(xor);
 ...
 */
static errr Term_wipe_x11(s32b x, s32b y, s32b n)
{
	/* Erase (use black) */
 ...
 */
static errr Term_text_x11(s32b x, s32b y, s32b n, byte a, cptr s)
{
	/* Draw the text */
	Infoclr_set(clr[a]);
 ...
 */
static errr term_data_init(term_data *td, s32b i)
{
	term *t = &td->t;
 ...
 */
errr init_x11(s32b argc, char *argv[])
{
	s32b i;
	...
	for (i = 1; i < argc; i++)
	{
		...
		if (prefix(argv[i], "-g"))
		{
			if (++i == argc)
			{
				printf("Argument missing for option -g\n");
				return -1;
			}
			...
			else if (prefix(argv[i], "16"))
			{
				arg_graphics_type = GRAPHICS_16X16;
			}
			else if (prefix(argv[i], "32"))
			{
				arg_graphics_type = GRAPHICS_32X32;
			}
			else
			{
				arg_graphics_type = GRAPHICS_NONE;
			}
			continue;
		}
