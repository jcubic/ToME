...
void evolve_level(bool noise)
{
	...
	if (noise)
	{
		for (i = 1; i < cur_wid - 1; i++)
		{
			...
			{
				if (has_flag(&f_info[cave[j][i].feat], FLAG_WALL)) cw++;
				if (has_flag(&f_info[cave[j][i].feat], FLAG_FLOOR)) cf++;
			}
		}
		...
		for (i = 1; i < cur_wid - 1; i++)
		{
			for (j = 1; j < cur_hgt - 1; j++)
			{
				...
				if (has_flag(&f_info[c_ptr->feat], FLAG_PERMANENT)) continue;

				/* Avoid evolving grids with object or monster */
				if (flag_used(&c_ptr->inventory) || c_ptr->m_idx) continue;
	...
	for (i = 1; i < cur_wid - 1; i++)
	{
		for (j = 1; j < cur_hgt - 1; j++)
		{
			...
			for (x = i - 1; x <= i + 1; x++)
			{
				...
				{
					if ((x == i) && (y == j)) continue;
					if (has_flag(&f_info[cave[y][x].feat], FLAG_WALL)) c++;
				}
			}
