 ...
 * Play a given cmovie
 */
s16b do_play_cmovie(cptr cmov_file, bool show_info)
{
	PHYSFS_file *fff;
	...
	/* Build the filename */
	path_build(buf, 1024, TENGINE_DIR_CMOV, cmov_file);
	...
	while (0 == my_fgets(fff, buf, 1024))
	{
		...
		if (buf[0] == 'P')
		{
			int  sx, sy, sa, sc; /* quite sscanf warnings */
			s32b  x,  y,  a,  c;
			...
			x = sx, y = sy, a = sa, c = sc; /* quite sscanf warnings */
...
void do_record_cmovie(cptr cmovie)
{
	...
	/* Build the filename */
	path_build(buf, 1024, TENGINE_DIR_CMOV, cmovie);
...
void do_start_cmovie()
{
	...
	if (get_string("Cmovie name: ", name, 80))
	{
		if (name[0] && (name[0] != ' '))
		{
			...
			else do_play_cmovie(rname, TRUE);
		}
	}
}


void cmovie_clean_line(s32b y, char *abuf, char *cbuf)
{
 ...
 */
void cmovie_record_line(s32b y)
{
	s32b i;
	char abuf[256];
	char cbuf[256];
	...
	/* TODO: need new version of cmovie spec for RGB color */

	/* Write a colour record */
	fprintf(movfile, "E:%d:");
	for(i = 0; i < 80; ++i)
		fprintf(movfile, "%c", conv_color[abuf[i] & 0xf]);
	fprintf(movfile, "\n");
