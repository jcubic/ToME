...
#define PHYSFS_LANG_GERMAN             8  /*  German by Michael Renner  */
 ...
 #define FLAT_ARCHIVE_DESCRIPTION "Flat archive format"
...
typedef struct __PHYSFS_FILEHANDLE__
{
    ...
    PHYSFS_uint8 forReading;
        ...
        /*
         * Non-null if file opened as transparently compressed
         */
    gzFile *gz_handle;
...
typedef struct __PHYSFS_DIRFUNCTIONS__
{
    ...
    int (*remove)(DirHandle *r, const char *filename);
        ...
        /*
         * Rename a file in the archive/directory.
         *  Return non-zero on success, zero on failure.
         *  This filename is in platform-independent notation.
         *  This method may be NULL.
         * On failure, call __PHYSFS_setError().
         */
    int (*rename)(DirHandle *r, const char *filename, const char *new_filename);
...
/*
 * Rename a file or directory entry in the actual filesystem. (path) and (newpath)
 *  are specified in platform-dependent notation.
 *
 * On error, return zero and set the error message. Return non-zero on success.
 */
int __PHYSFS_platformRename(const char *path, const char *newpath);
