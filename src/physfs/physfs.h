...
#elif (defined PHYSFS_BCC_64BIT_SUPPORT)
typedef unsigned long         PHYSFS_uint64;
typedef signed long           PHYSFS_sint64;
#else
...
#if (!defined(PHYSFS_NO_64BIT_SUPPORT)) && (!defined(PHYSFS_BCC_64BIT_SUPPORT))
...
/**
 * \fn int PHYSFS_rename(const char *filename, const char *new_filename)
 * \brief Rename/move a file or directory.
 *
 * (filename) is specified in platform-independent notation in relation to the
 *  write dir.
 * (new_filename) is specified in platform-independent notation in relation to the
 *  write dir.
 *
 *  \param filename Filename to rename.
 *  \param new_filename Destination filename.
 *  \return nonzero on success, zero on error. Specifics of the error can be
 *          gleaned from PHYSFS_getLastError().
 */
__EXPORT__ int PHYSFS_rename(const char *filename, const char *new_filename);
 ...
 * \sa PHYSFS_close
 */
__EXPORT__ PHYSFS_file *PHYSFS_openWrite(const char *filename, int compress);
 ...
 * \sa PHYSFS_close
 */
__EXPORT__ PHYSFS_file *PHYSFS_openAppend(const char *filename, int compress);
 ...
 * \sa PHYSFS_close
 */
__EXPORT__ PHYSFS_file *PHYSFS_openRead(const char *filename, int compress);
...
/**
 * \fn char *PHYSFS_getDependentPath(const char *fname)
 * \brief Convert a physfs filename to a real system filename
 *
 *    \param file PhysicsFS filename to convert.
 *   \return NULL on failure, newly allocated string on success.
 */
__EXPORT__ char *PHYSFS_getDependentPath(const char *fname);

/*
 * DG - Mod from official physfs
 * allows mounting a zip file to anywhere in the directory structure
 */
