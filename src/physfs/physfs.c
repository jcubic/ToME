...
#if (defined PHYSFS_SUPPORTS_FLAT)
extern const PHYSFS_ArchiveInfo   __PHYSFS_ArchiveInfo_FLAT;
extern const DirFunctions         __PHYSFS_DirFunctions_FLAT;
#endif
...
/* Makes the next archive mounted in the specified directory */
char *PHYSFS_specific_mount = NULL;
...
int PHYSFS_rename(const char *fname, const char *new_fname)
{
    int retval;
    DirHandle *h;

    BAIL_IF_MACRO(fname == NULL, ERR_INVALID_ARGUMENT, 0);
    BAIL_IF_MACRO(new_fname == NULL, ERR_INVALID_ARGUMENT, 0);
    while (*fname == '/')
        fname++;
    while (*new_fname == '/')
        new_fname++;
    ...
    h = writeDir->dirHandle;
    BAIL_IF_MACRO_MUTEX(!__PHYSFS_verifySecurity(h,fname,0),NULL,stateLock,0);
    BAIL_IF_MACRO_MUTEX(!__PHYSFS_verifySecurity(h,new_fname,0),NULL,stateLock,0);
    retval = h->funcs->rename(h, fname, new_fname);
...
static PHYSFS_file *doOpenWrite(const char *fname, int appending, int compress)
{
	...
	{
		char mode[6];
		sprintf(mode, "%cb%d", (appending)?'a':'w', compress);
		rc->gz_handle = gzdopen(retval, mode);
	}
    } /* else */
...
PHYSFS_file *PHYSFS_openWrite(const char *filename, int compress)
{
    return(doOpenWrite(filename, 0, compress));
} /* PHYSFS_openWrite */


PHYSFS_file *PHYSFS_openAppend(const char *filename, int compress)
{
    return(doOpenWrite(filename, 1, compress));
} /* PHYSFS_openAppend */


PHYSFS_file *PHYSFS_openRead(const char *fname, int compress)
{
    PHYSFS_file *retval = NULL;
    FileHandle *rc = NULL;
    FileHandleList *list;
                          ...
                          PHYSFS_uint32 objSize, PHYSFS_uint32 objCount)
{
    ...
    if (h->gz_handle)
    {
	    /* Call the gz API and trick ourselves into not doing infinite loop */
	    gzFile handle = h->gz_handle;
	    int rc;
	    h->gz_handle = NULL;
	    rc = gzread(handle, buffer, objSize * objCount);
	    h->gz_handle = handle;
	    return rc;
    }
    else
                           ...
                           PHYSFS_uint32 objSize, PHYSFS_uint32 objCount)
{
    ...
    if (h->gz_handle)
    {
	    ...
	    int rc;
	    h->gz_handle = NULL;
	    rc = gzwrite(handle, (void*)buffer, objSize * objCount);
	    h->gz_handle = handle;
	    return rc;
    }
    else
...
int PHYSFS_flush(PHYSFS_file *handle)
{
    ...
    if (h->gz_handle)
    {
	    ...
	    int rc;
	    h->gz_handle = NULL;
	    rc = gzflush(handle, Z_SYNC_FLUSH);
	    h->gz_handle = handle;
	    return (rc == 0) ? 1 : 0;
    }
    else
