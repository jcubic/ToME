...
static int ZIP_rename(DirHandle *h, const char *name, const char *new_name);
...
static int zip_load_entry(void *in, ZIPentry *entry, PHYSFS_uint32 ofs_fixup)
{
     ...
     */
    if (PHYSFS_specific_mount != NULL)
    {
	    char *old = entry->name;
	    char *n = (char *) malloc(fnamelen + 1 + strlen(PHYSFS_specific_mount));
	    strcpy(n, PHYSFS_specific_mount);
	    strcat(n, entry->name);
	    entry->name = n;
	    free(old);
    }
