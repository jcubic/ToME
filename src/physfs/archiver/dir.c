...
static int DIR_rename(DirHandle *h, const char *name, const char *new_name);
...
{
    char *f = __PHYSFS_platformCvtToDependent((char *)(h->opaque), name, NULL);
    char *new_f = __PHYSFS_platformCvtToDependent((char *)(h->opaque), new_name, NULL);
    int retval;

    BAIL_IF_MACRO((f == NULL) || (new_f == NULL), NULL, 0);
    retval = __PHYSFS_platformRename(f, new_f);
    free(f);
    free(new_f);
    return(retval);
} /* DIR_remove */
