	...
	if (!(noscore & 0x00FF))
	{
		...
		if (dun_level)
		{
			...
			for (i = 0; i < 5; i++)
			{
				...
				/* Build the filename */
				path_build(str, 1024, TENGINE_DIR_BONE, tmp);
...
static s32b ghost_race;
static s32b ghost_class;
 ...
 */
static void ghost_blow(s32b i, s32b m, s32b e, s32b d, s32b s)
{
...
static void set_ghost_aux_1(void)
{
	...
	s32b attack1, attack2;

	s32b lev = r_ptr->level;

	s32b grace = ghost_race;
	s32b gclass = ghost_class;
	...
	sprintf(r_ptr->name, "%s, the skeletal %s %s",
...
static void set_ghost_aux_2(void)
{
	...
	case 3:
		{
			sprintf(r_ptr->name, "%s, the Skeleton %s", gb_name, gr_name);
			r_ptr->d_char = 's';
			r_ptr->d_attr = TERM_WHITE;
			r_ptr->flags2 |= (RF2_OPEN_DOOR | RF2_BASH_DOOR);
			r_ptr->flags3 |= (RF3_IM_COLD);
	...
	case 5:
		{
			sprintf(r_ptr->name, "%s, the Zombified %s", gb_name, gr_name);
			r_ptr->d_char = 'z';
	...
	case 7:
		{
			sprintf(r_ptr->name, "%s, the Mummified %s", gb_name, gr_name);
			r_ptr->d_char = 'z';
			r_ptr->d_attr = TERM_L_DARK;
	...
	case 8:
		{
			sprintf(r_ptr->name, "%s, the Poltergeist", gb_name);
			r_ptr->d_char = 'G';
			r_ptr->d_attr = TERM_WHITE;
	...
	case 10:
		{
			sprintf(r_ptr->name, "%s, the Spirit", gb_name);
			r_ptr->d_char = 'G';
			r_ptr->d_attr = TERM_WHITE;
			r_ptr->flags1 |= (RF1_DROP_1D2);
			r_ptr->flags2 |= (RF2_INVISIBLE | RF2_PASS_WALL);
	...
	case 11:
		{
			sprintf(r_ptr->name, "%s, the Ghost", gb_name);
			r_ptr->d_char = 'G';
			r_ptr->d_attr = TERM_WHITE;
			r_ptr->flags1 |= (RF1_DROP_1D2);
			r_ptr->flags2 |= (RF2_INVISIBLE | RF2_PASS_WALL);
	...
	case 12:
		{
			sprintf(r_ptr->name, "%s, the Vampire", gb_name);
			r_ptr->d_char = 'V';
	...
	case 13:
		{
			sprintf(r_ptr->name, "%s, the Wraith", gb_name);
			r_ptr->d_char = 'W';
			r_ptr->d_attr = TERM_WHITE;
	...
	case 14:
		{
			sprintf(r_ptr->name, "%s, the Vampire Lord", gb_name);
			r_ptr->d_char = 'V';
			r_ptr->d_attr = TERM_BLUE;
	...
	case 17:
		{
			sprintf(r_ptr->name, "%s, the Lich", gb_name);
			r_ptr->d_char = 'L';
 ...
 */
static void set_ghost(cptr pname, s32b hp, s32b grace, s32b gclass, s32b lev, bool town)
{
	s32b i;
...
{
#if 0 /* DGDGDGDG */
	s32b y, x, hp, level, grace, gclass;
	...
	/* Choose a bones file */
	sprintf(tmp, "%s%sbone%03d.%03d", TENGINE_DIR_BONE, PATH_SEP, dungeon_type, level);
