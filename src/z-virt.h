...
/* Free an array of N things of type T at P */
#define C_FREE(P,N,T) \
	(void)(rnfree(P,C_SIZE(N,T)))

/* Free one thing of type T at P */
#define FREE(P,T) \
	(void)(rnfree(P,SIZE(T)))

/* Free an array of N things of type T at P, return NULL */
#define NULL_C_FREE(P,N,T) \
	(T*)(rnfree(P,C_SIZE(N,T)))

/* Free one thing of type T at P, return NULL */
#define NULL_FREE(P,T) \
	(T*)(rnfree(P,SIZE(T)))
...
/* Realloc an array of type T, at location P,of size N, to an array of size N+ADD */
#define RE_C_MAKE(P, N, ADD, T) 		\
	{ 					\
	T *tmp;					\
	(void) C_MAKE(tmp, N + ADD, T);		\
	(void) C_COPY(tmp, P, N, T);                   \
	C_FREE(P, N, T);                        \
	P = tmp;                                \
	}
...
extern vptr (*rnfree_aux)(vptr, size_t);
...
extern vptr rnfree(vptr p, size_t len);

/* Panic, attempt to Allocate 'len' bytes */
extern vptr rpanic(size_t len);

/* Allocate (and return) 'len', or dump core */
extern vptr ralloc(size_t len);
