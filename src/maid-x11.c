...
static byte get_byte(PHYSFS_file *fff)
{
	/* Get a character, and return it */
	return (fgetc(fff) & 0xFF);
}

static void rd_byte(PHYSFS_file *fff, byte *ip)
{
	*ip = get_byte(fff);
}

static void rd_u16b(PHYSFS_file *fff, u16b *ip)
{
	(*ip) = get_byte(fff);
	(*ip) |= ((u16b)(get_byte(fff)) << 8);
}

static void rd_u32b(PHYSFS_file *fff, u32b *ip)
{
	(*ip) = get_byte(fff);
...
static XImage *ReadBMP(Display *dpy, char *Name)
{
	...
	{
		s32b y2 = infoheader.biHeight - y - 1;
		...
		for (x = 0; x < infoheader.biWidth; x++)
		{
			...
			if (PHYSFS_eof(f)) quit_fmt("Unexpected end of file in %s", Name);
			ch = fgetc(f);
...
static s32b redShift, greenShift, blueShift;
 ...
 */
static void GetScaledRow(XImage *Im, s32b x, s32b y, s32b iw, s32b ow,
                         unsigned long *redScan, unsigned long *greenScan,
                         unsigned long *blueScan)
{
	s32b xi, si, sifrac, ci, cifrac, addWhole, addFrac;
	unsigned long pix;
	s32b prevRed, prevGreen, prevBlue, nextRed, nextGreen, nextBlue;
	bool getNextPix;
 ...
 * are divided first.
 */
static void PutRGBScan(XImage *Im, s32b x, s32b y, s32b w, s32b div,
                       unsigned long *redScan, unsigned long *greenScan,
                       unsigned long *blueScan)
{
	s32b xi;
...
static void ScaleIcon(XImage *ImIn, XImage *ImOut,
                      s32b x1, s32b y1, s32b x2, s32b y2,
                      s32b ix, s32b iy, s32b ox, s32b oy)
{
	s32b div;
	s32b xi, yi, si, sifrac, ci, cifrac, addWhole, addFrac;
