 ...
 */
static guint32 angband_colours[256];
 ...
 */
static cptr get_default_font(s32b term)
{
	char buf[64];
	cptr font_name;
...
#else
static vptr hook_rnfree(vptr v, size_t size)
#endif /* ANGBAND300 */
{
	/* Dispose */
	g_free(v);
 ...
 */
static vptr hook_ralloc(size_t size)
{
	/* Make a new pointer */
 ...
 */
static s32b graf_mode = GRAPHICS_NONE;
static s32b graf_mode_request = GRAPHICS_NONE;
 ...
 */
static s32b tile_rows;
static s32b tile_cols;


/*
 * Directory name(s)
 */
static cptr TENGINE_DIR_XTRA_GRAF;
...
static void get_scaled_row(
        ...
        rgb_type *scan)
{
	s32b xi, si, sifrac, ci, cifrac, add_whole, add_frac;
	guint32 pix;
	rgb_type prev;
	rgb_type next;
	bool get_next_pix;
...
{
	s32b div;
	s32b xi, yi, si, sifrac, ci, cifrac, add_whole, add_frac;
        ...
        s32b oy)
{
	...
	s32b old_wid, old_hgt;

	s32b new_wid, new_hgt;

	s32b add, remainder, rem_tot, offset;
...
static GdkRGBImage *load_xpm(cptr filename)
{
	...
	s32b width, height, colours, chars;
	s32b i, j, k;
	bool ret;
	pal_type *pal = NULL;
	...
	path_build(buf, 1024, TENGINE_DIR_XTRA_GRAF, filename);
...
GdkRGBImage *load_bmp(cptr filename)
{
	...
	path_build(path, 1024, TENGINE_DIR_XTRA_GRAF, filename);
...
static void init_graphics(void)
{
	...
	if ((graf_mode_request == GRAPHICS_NONE) ||
	                ...
	                !graf_init(tile_name, graf_wid, graf_hgt))
	{
		...
		/* reject requests */
		graf_mode_request = GRAPHICS_NONE;
		smooth_rescaling_request = smooth_rescaling;
 ...
 */
static errr Term_wipe_gtk(s32b x, s32b y, s32b n)
{
	term_data *td = (term_data*)(Term->data);
 ...
 */
static errr Term_text_gtk(s32b x, s32b y, s32b n, byte a, cptr s)
{
	term_data *td = (term_data*)(Term->data);
 ...
 */
static errr Term_curs_gtk(s32b x, s32b y)
{
	term_data *td = (term_data*)(Term->data);
	s32b cells = 1;
 ...
 */
static errr Term_xtra_gtk(s32b n, s32b v)
{
        ...
        gpointer user_data)
{
	...
	graf_mode_request = (s32b)user_data;
        ...
        gpointer user_data)
{
	...
	path_build(buf, 1024, TENGINE_DIR_SAVE, "*");
...
static GtkItemFactoryEntry main_menu_items[] =
{
	...
	{ "/Options/Graphics/8x8", NULL,
	  change_graf_mode_event_handler, GRAPHICS_8X8, "<CheckItem>" },
	{ "/Options/Graphics/16x16", NULL,
	  change_graf_mode_event_handler, GRAPHICS_16X16, "<CheckItem>" },
	{ "/Options/Graphics/32x32", NULL,
	  change_graf_mode_event_handler, GRAPHICS_32X32, "<CheckItem>" },
# ifdef USE_DOUBLE_TILES
...
{
	s32b i;
	s32b nmenu_items = sizeof(main_menu_items) / sizeof(main_menu_items[0]);
	...
	for (i = 0; i < MAX_TERM_DATA; i++)
	{
		...
		term_entry[i].path = (cptr)string_make(buf);

		/* XXX XXX Build the real path name to the entry */
		strnfmt(buf, 64, "/Options/Font/%s", angband_term_name[i]);

		/* XXX XXX Store it in the menu definition */
		font_entry[i].path = (cptr)string_make(buf);
	}
}
        ...
        gpointer user_data)
{
	...
	check_menu_item(
	        ...
	        (graf_mode == GRAPHICS_NONE));
	check_menu_item(
	        "<Angband>/Options/Graphics/8x8",
	        (graf_mode == GRAPHICS_8X8));
	check_menu_item(
	        "<Angband>/Options/Graphics/16x16",
	        (graf_mode == GRAPHICS_16X16));
	check_menu_item(
	        "<Angband>/Options/Graphics/32x32",
	        (graf_mode == GRAPHICS_32X32));
 ...
 */
static void init_gtk_window(term_data *td, s32b i)
{
...
# ifdef USE_GRAPHICS
	...
	if (TENGINE_DIR_XTRA_GRAF) string_free(TENGINE_DIR_XTRA_GRAF);
 ...
 */
errr init_gtk2(s32b argc, char **argv)
{
	s32b i;
	...
	for (i = 1; i < argc; i++)
	{
		...
		if (prefix(argv[i], "-g"))
		{
			...
			if (prefix(argv[i], "8"))
			{
				graf_mode_request = GRAPHICS_8X8;
			}
			else if (prefix(argv[i], "16"))
			{
				graf_mode_request = GRAPHICS_16X16;
			}
			else if (prefix(argv[i], "32"))
			{
				graf_mode_request = GRAPHICS_32X32;
			}
			else
			{
				graf_mode_request = GRAPHICS_NONE;
...
# endif  /* USE_DOUBLE_TILES */
...

	{
		...
		/* Build the "graf" path */
		path_build(path, 1024, TENGINE_DIR_XTRA, "graf");

		/* Allocate the path */
		TENGINE_DIR_XTRA_GRAF = string_make(path);
	}
