/*
** $Id: lparser.c,v 1.12 2006/02/15 00:04:10 darkgod Exp $
...
static const struct {
  ...
  char right; /* right priority */
   ...
   {6, 6}, {6, 6}, {6, 6},          /* DG -- Bitwise operators*/
   {2, 2}, {2, 2},                  /* equality */
...
static void realforlist (LexState *ls, TString *valname) {
  /* realforlist -> NAME IN exp1 forbody */
  next(ls);  /* skip `in' */
  exp1(ls);  /* table */
  new_localvarstr(ls, "(table)", 0);
  new_localvarstr(ls, "(n)", 1);
  new_localvarstr(ls, "(index)", 2);
  new_localvar(ls, valname, 3);
  forbody(ls, 4, OP_LISTFORPREP, OP_LISTFORLOOP);
}


static void forstat (LexState *ls, int line) {
  /* forstat -> fornum | forlist | realforlist */
  FuncState *fs = ls->fs;
  TString *varname;
  Breaklabel bl;
  enterbreak(fs, &bl);
  ...
  switch (ls->t.token) {
    ...
    /* avoids `in' being a reserved word */
    case TK_NAME: {
      if (ls->t.seminfo.ts == luaS_new(ls->L, "in")) {
        realforlist(ls, varname);
        break;
      }
      /* NOTE: falling through case */
    }
    default: luaK_error(ls, "`=' or `,' or `in' expected");
  }
...
static int funcname (LexState *ls, expdesc *v) {
  ...
  int needself = 0;
  singlevar(ls, str_checkname(ls), v);
  while (!needself && (ls->t.token == ':' || ls->t.token == '.')) {
    needself = (ls->t.token == ':');
    next(ls);
    luaK_tostack(ls, v, 1);
    luaK_kstr(ls, checkname(ls));
