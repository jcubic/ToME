...
** TeCGraf/PUC-Rio
** Jul 1998
** $Id: tolua_rg.c,v 1.5 2006/11/16 10:39:44 darkgod Exp $
*/
...
/* Special function tag method for arrays to index arrays starting at 0 */
static int toluaI_get_array_as_fct(lua_State* L)
{
...
void tolua_globalarray (lua_State* L,char* name, lua_CFunction get, lua_CFunction set)
{
 ...
 lua_pushcfunction(L,toluaI_get_array_as_fct);
...
void tolua_tablearray(lua_State* L, char* table, char* name, lua_CFunction get, lua_CFunction set)
{
 int tag = lua_newtag(L);
 lua_getglobal(L,table);
 lua_pushstring(L,".array");
