 ...
 *
 *
 * For examples below, assume "s32b n = 0; s32b m = 100; char buf[100];",
 * plus "char *s = NULL;", and unknown values "char *txt; s32b i;".
 *
 ...
 */
typedef size_t (*vstrnfmt_aux_func)(char *buf, size_t max, cptr fmt, vptr arg);

/*
 * The "default" user defined print routine.  Ignore the "fmt" string.
 */
static size_t vstrnfmt_aux_dflt(char *buf, size_t max, cptr fmt, vptr arg)
{
	size_t len;
	char tmp[32];
 ...
 */
size_t strfmt(char *buf, cptr fmt, ...)
{
	size_t len;
