...
void squeltch_inventory(void)
{
	...
	while (found && num_iter ++ < 100)
	{
		...
		for_inventory(p_ptr, o_ptr, 0, INVEN_PACK);
		{
			/* Push the function */
			lua_settop(L, oldtop);
			lua_getglobal(L, "apply_rules");
...
/********************** The interface **********************/
static cptr *get_rule_list(s32b *max)
{
	cptr *list;
	s32b i;

	*max = exec_lua("return auto.rules_max");
	C_MAKE(list, *max, cptr);

	for (i = 0; i < *max; i++)
	{
		list[i] = string_exec_lua(format("return auto.rules[%d].table.args.name", i));
	}
...
/* Create a new rule */
static s32b automatizer_new_rule()
{
	s32b wid, hgt, max, begin = 0, sel = 0;
	s32b c;
	...
	while (1)
	{
		...
		display_list(0, 0, hgt - 1, 15, "Rule types", types_list,
					 max, begin, sel, TERM_L_GREEN, FALSE);
...
static void adjust_begin(s32b *begin, s32b *sel, s32b max, s32b hgt)
{
	if (*sel < 0)
	{
		*sel = max - 1;
...
void do_cmd_automatizer()
{
	...
	s32b c;
	cptr *list = NULL;
	s32b  max, begin = 0, sel = 0;
	s32b  active = ACTIVE_LIST;
	cptr keys;
	cptr keys2;
	cptr keys3;
	...
	while (1)
	{
		...
		display_list(0, 0, hgt - 1, 15, "Rules", list, max, begin, sel,
					 (active == ACTIVE_LIST) ? TERM_L_GREEN : TERM_GREEN,
					 FALSE);
		...
		if (max) exec_lua(format("xml.write_out_y = 1; xml.write_out_x = 16; xml.write_out_h = %d; xml.write_out_w = %d; xml.write_y = 0; xml.write_x = 0; xml:display_xml(auto.rules[%d].table, '')", hgt - 4 - 1, wid - 1 - 15 - 1, sel));
		...
		if (active == ACTIVE_LIST)
		{
			...
			else if (c == 'r')
			{
				...
				sprintf(name, string_exec_lua(format("return auto.rules[%d].table.args.name", sel)));
