...
/*
 * This file helps ToME work with Windows computers.
 *
 * To use this file, use an appropriate "Makefile" or "Project File",
 * make sure that "USE_WINDOWS" and/or "WIN32" are defined somewhere, and
 ...
 * several extra files not distributed with the standard ToME code.
...
#define IDM_OPTIONS_GRAPHICS_8X8	400
#define IDM_OPTIONS_GRAPHICS_16X16	401
#define IDM_OPTIONS_GRAPHICS_32X32	404
#define IDM_OPTIONS_GRAPHICS_ASCII  403
 ...
 * Directory names
 */
static cptr TENGINE_DIR_XTRA_SOUND;
static cptr TENGINE_DIR_XTRA_HELP;
 ...
 */
static char *analyze_font(char *path, s32b *wp, s32b *hp)
{
	s32b wid, hgt;
...
static void term_getsize(term_data *td)
{
	...
	/*
	 * The main window is always at least 80x24. I'm not sure if
	...
	/* Window sizes */
	wid = (td->cols + td->size_ow1) * td->tile_wid;
	hgt = (td->rows + td->size_oh1 + 1) * td->tile_hgt;
...
static void save_prefs(void)
{
	...
	esettings_set_int("win.graphics.enabled", arg_graphics);

	/* Save the "arg_bigtile" flag */
	esettings_set_int("win.graphics.bigtile", arg_bigtile ? TRUE : FALSE);

	/* Save the "arg_sound" flag */
	esettings_set_int("win.sound.enabled", arg_sound);

#ifdef SUPPORT_GAMMA
	/* Save the "gamma_val" */
	esettings_set_int("win.graphics.gammaval", gamma_val);
#endif /* SUPPORT_GAMMA */
	...
	for (i = 0; i < MAX_TERM_DATA; ++i)
	{
		...
		/* Visible */
		sprintf(heading, "win.term%d.visible", i );
		esettings_set_int(heading, td->visible ? 1 : 0);

		/* Font */
		sprintf(heading, "win.term%d.font", i );
		esettings_set_str(heading, td->font_file ? td->font_file : DEFAULT_FONT);

		/* Bizarre */
		sprintf(heading, "win.term%d.bizarre", i );
		esettings_set_int(heading, td->bizarre ? 1 : 0);

		/* Tile size (x) */
		sprintf(heading, "win.term%d.tile_width", i );
		esettings_set_int(heading, td->tile_wid);

		/* Tile size (y) */
		sprintf(heading, "win.term%d.tile_height", i );
		esettings_set_int(heading, td->tile_hgt);

		/* Window size (x) */
		sprintf(heading, "win.term%d.columns", i );
		esettings_set_int(heading, td->cols);

		/* Window size (y) */
		sprintf(heading, "win.term%d.rows", i );
		esettings_set_int(heading, td->rows);

		/* Acquire position */
		GetWindowRect(td->w, &rc);

		/* Window position (x) */
		sprintf(heading, "win.term%d.pos.x", i );
		esettings_set_int(heading, rc.left);

		/* Window position (y) */
		sprintf(heading, "win.term%d.pos.y", i );
		esettings_set_int(heading, rc.top);
	}
}
...
static void load_prefs(void)
{
	...
	arg_graphics = esettings_get_int("win.graphics.enabled", GRAPHICS_NONE);

	/* Extract the "arg_bigtile" flag */
	arg_bigtile = esettings_get_int("win.graphics.bigtile", FALSE);
	use_bigtile = arg_bigtile;

	/* Extract the "arg_sound" flag */
	arg_sound = esettings_get_int("win.sound.enabled", 0);

#ifdef SUPPORT_GAMMA
	/* Extract the "gamma_val" */
	gamma_val = esettings_get_int("win.graphics.gammaval", 0);
#endif /* SUPPORT_GAMMA */
	...
	for (i = 0; i < MAX_TERM_DATA; ++i)
	{
		...
		/* Visible */
		sprintf(heading, "win.term%d.visible", i );
		td->visible = esettings_get_int(heading, 0);

		/* Bizarre */
		sprintf(heading, "win.term%d.bizarre", i );
		td->bizarre = esettings_get_int(heading, 0);

		/* Desired font */
		sprintf(heading, "win.term%d.font", i );
		tmp = esettings_get_str(heading, DEFAULT_FONT);
		...
		/* Tile size */
		sprintf(heading, "win.term%d.tile_width", i );
		td->tile_wid = esettings_get_int(heading, 8);
		sprintf(heading, "win.term%d.tile_height", i );
		td->tile_hgt = esettings_get_int(heading, 13);

		/* Window size */
		sprintf(heading, "win.term%d.columns", i );
		td->cols = esettings_get_int(heading, 0);
		sprintf(heading, "win.term%d.rows", i );
		td->rows = esettings_get_int(heading, 0);

		/* Window position */
		sprintf(heading, "win.term%d.pos.x", i );
		td->pos_x = esettings_get_int(heading, 0);
		sprintf(heading, "win.term%d.pos.y", i );
		td->pos_y = esettings_get_int(heading, 0);
	}
}
 ...
 * "win_clr[]" which is used for the basic 16 terminal colors.
 *
...
static bool init_graphics()
{
	...
	/*if (can_use_graphics != arg_graphics) */
	{
		...
		if (arg_graphics == GRAPHICS_32X32)
		{
			wid = 32;
			hgt = 32;
		...
		else if (arg_graphics == GRAPHICS_16X16)
		{
			wid = 16;
			hgt = 16;
		...
		path_build(buf, PATH_MAX, TENGINE_DIR_GRAF, name);

		/* Load the bitmap or quit */
		if (!ReadDIB(data[0].w, PHYSFS_getDependentPath(buf), &infGraph))
		{
		...
		/* only use mask for 16x16 and 32x32 */
		if ((arg_graphics == GRAPHICS_32X32) || (arg_graphics == GRAPHICS_16X16))
		{
			/* Access the bitmap file */
			path_build(buf, PATH_MAX, TENGINE_DIR_GRAF, mask);

			/* Load the bitmap or quit */
			if (!ReadDIB(data[0].w, PHYSFS_getDependentPath(buf), &infMask))
			{
				plog_fmt("Cannot read mask file '%s'", mask);
				return (FALSE);
			}
		}
...
static bool init_sound()
{
	...
	if (!can_use_sound)
	{
		...
		for (i = 1; i < SOUND_MAX; i++)
		{
			...
			/* Access the sound */
			path_build(buf, PATH_MAX, TENGINE_DIR_XTRA_SOUND, wav);
...
static void term_change_font(term_data *td)
{
	...
	strcpy(buf, PHYSFS_getDependentPath(TENGINE_DIR_FONT));

	/* Ask for a choice */
	memset(&ofn, 0, sizeof(ofn));
	ofn.lStructSize = sizeof(ofn);
	ofn.hwndOwner = data[0].w;
	ofn.lpstrFilter = "Term Font Files (*.fon)\0*.fon\0";
	ofn.nFilterIndex = 1;
	ofn.lpstrFile = tmp;
	ofn.nMaxFile = 128;
	...
	if (GetOpenFileName(&ofn))
	{
		...
		if (term_force_font(td, tmp))
		{
			...
			path_build(tmp, PATH_MAX, TENGINE_DIR_FONT, DEFAULT_FONT);

			/* Force the use of that font */
			(void)term_force_font(td, PHYSFS_getDependentPath(tmp));
		}
 ...
 */
static errr Term_user_win(s32b n)
{
	/* Success */
	return (0);
}
 ...
 */
static errr Term_xtra_win_event(s32b v)
{
	MSG msg;
 ...
 * Hack -- make a sound
 */
static errr Term_xtra_win_sound(s32b v)
{
	/* Sound disabled */
 ...
 */
static s32b Term_xtra_win_delay(s32b v)
{
 ...
 * Do a "special thing"
 */
static errr Term_xtra_win(s32b n, s32b v)
{
 ...
 */
static errr Term_curs_win(s32b x, s32b y)
{
	term_data *td = (term_data*)(Term->data);
 ...
 */
static errr Term_wipe_win(s32b x, s32b y, s32b n)
{
	term_data *td = (term_data*)(Term->data);
 ...
 */
static errr Term_text_win(s32b x, s32b y, s32b n, byte a, const char *s)
{
	term_data *td = (term_data*)(Term->data);
	RECT rc;
	HDC hdc;
...
static errr Term_pict_win(s32b x, s32b y, s32b n, const byte *ap, const char *cp, const byte *tap, const char *tcp, const byte *eap, const char *ecp)
# else /* USE_EGO_GRAPHICS */
static errr Term_pict_win(s32b x, s32b y, s32b n, const byte *ap, const char *cp, const byte *tap, const char *tcp)
# endif  /* USE_EGO_GRAPHICS */
# else /* USE_TRANSPARENCY */
static errr Term_pict_win(s32b x, s32b y, s32b n, const byte *ap, const char *cp)
# endif  /* USE_TRANSPARENCY */
{
	term_data *td = (term_data*)(Term->data);
...
# ifdef USE_TRANSPARENCY
	...
	if (arg_graphics == GRAPHICS_16X16 || arg_graphics == GRAPHICS_32X32)
	{
...
static void init_windows(void)
{
	...
	for (i = 0; i < MAX_TERM_DATA; i++)
	{
		...
		if (td->font_want)
		{
			path_build(buf, PATH_MAX, TENGINE_DIR_FONT, td->font_want);
		}
		else
		{
			strcpy(buf, TENGINE_DIR_FONT);
		}

		/* Activate the chosen font */
		if (term_force_font(td, PHYSFS_getDependentPath(buf)))
		{
			/* Force the use of the default font */
			path_build(buf, PATH_MAX, TENGINE_DIR_FONT, "8x13.fon");
			(void)term_force_font(td, PHYSFS_getDependentPath(buf));
	...
	td->w = CreateWindowEx(td->dwExStyle, AppName,
	                       ...
	                       HWND_DESKTOP, NULL, hInstance, NULL);
	my_td = NULL;
	if (!td->w) quit("Failed to create main window");
...
static void setup_menus(void)
{
	...
	EnableMenuItem(hm, IDM_OPTIONS_GRAPHICS_8X8,
	               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	EnableMenuItem(hm, IDM_OPTIONS_GRAPHICS_16X16,
	               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	EnableMenuItem(hm, IDM_OPTIONS_GRAPHICS_32X32,
	               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	EnableMenuItem(hm, IDM_OPTIONS_GRAPHICS_ASCII,
	               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	...
	CheckMenuItem(hm, IDM_OPTIONS_GRAPHICS_8X8,
	              (arg_graphics == GRAPHICS_8X8 ? MF_CHECKED : MF_UNCHECKED));
	CheckMenuItem(hm, IDM_OPTIONS_GRAPHICS_16X16,
	              (arg_graphics == GRAPHICS_16X16 ? MF_CHECKED : MF_UNCHECKED));
	CheckMenuItem(hm, IDM_OPTIONS_GRAPHICS_32X32,
	              (arg_graphics == GRAPHICS_32X32 ? MF_CHECKED : MF_UNCHECKED));
	CheckMenuItem(hm, IDM_OPTIONS_GRAPHICS_ASCII,
	...
	EnableMenuItem(hm, IDM_OPTIONS_GRAPHICS_8X8, MF_ENABLED);
	/* Menu "Options", Item "Graphics" */
	EnableMenuItem(hm, IDM_OPTIONS_GRAPHICS_16X16, MF_ENABLED);
	/* Menu "Options", Item "Graphics" */
	EnableMenuItem(hm, IDM_OPTIONS_GRAPHICS_32X32, MF_ENABLED);
	/* Menu "Options", Item "Graphics" */
	EnableMenuItem(hm, IDM_OPTIONS_GRAPHICS_ASCII, MF_ENABLED);
	/* Menu "Options", Item "Graphics" */
...
/*
 * Check for double clicked (or dragged) module
 *
 * Apparently, Windows copies the entire filename into the first
 * piece of the "command line string".
 */
static void check_for_module_file(LPSTR cmd_line)
{
	char *s, *p;
	...
	if (!*s) return;
        ...
        /* Extract filename, it seems that it loves surrounding the path with "".. pfft */
        if (*s == '"') s++;
	if (s[strlen(s) - 1] == '"') s[strlen(s) - 1] = '\0';

	/* Ok ready, force the module loading */
        force_module = string_make(s);
}
...
ofn.lStructSize = sizeof(OPENFILENAME);
				...
				ofn.nMaxFile = 1024;
				ofn.lpstrInitialDir = TENGINE_DIR_SAVE;
				ofn.Flags = OFN_FILEMUSTEXIST | OFN_NOCHANGEDIR;
	...
	case IDM_FILE_SCORE:
		{
			...
			path_build(buf, sizeof(buf), TENGINE_DIR_SCORES, "scores.raw");
	...
	case IDM_OPTIONS_GRAPHICS_16X16:
		{
			/* Paranoia */
			if (!inkey_flag)
			{
	...
	case IDM_OPTIONS_GRAPHICS_32X32:
		{
			/* Paranoia */
			if (!inkey_flag)
			{
	...
	case IDM_OPTIONS_GRAPHICS_ASCII:
		{
			/* Paranoia */
			if (!inkey_flag)
			{
	...
	case IDM_HELP_GENERAL:
		{
			char buf[PATH_MAX + 1];
			char tmp[PATH_MAX + 1];
			path_build(tmp, PATH_MAX, TENGINE_DIR_XTRA_HELP, "angband.hlp");
			if (check_file(tmp))
			{
	...
	case IDM_HELP_SPOILERS:
		{
			char buf[PATH_MAX + 1];
			char tmp[PATH_MAX + 1];
			path_build(tmp, PATH_MAX, TENGINE_DIR_XTRA_HELP, "spoilers.hlp");
			if (check_file(tmp))
			{
				sprintf(buf, "winhelp.exe %s", tmp);
				WinExec(buf, SW_NORMAL);
...
#ifdef __MWERKS__
LRESULT FAR PASCAL ToMEWndProc(HWND hWnd, UINT uMsg,
...
#ifdef __MWERKS__
LRESULT FAR PASCAL ToMEListProc(HWND hWnd, UINT uMsg,
                                   WPARAM wParam, LPARAM lParam);
LRESULT FAR PASCAL ToMEListProc(HWND hWnd, UINT uMsg,
                                   WPARAM wParam, LPARAM lParam)
#else /* __MWERKS__ */
...
#ifdef __MWERKS__
LRESULT FAR PASCAL ToMESaverProc(HWND hWnd, UINT uMsg,
                                    WPARAM wParam, LPARAM lParam);
LRESULT FAR PASCAL ToMESaverProc(HWND hWnd, UINT uMsg,
                                    WPARAM wParam, LPARAM lParam)
#else /* __MWERKS__ */
...
static void init_stuff(void)
{
	...
	init_file_paths(NULL);


	/*** Validate the paths to ensure we have a working install ***/

	validate_dir(TENGINE_DIR_SCORES);
	validate_dir(TENGINE_DIR_BONE);
	validate_dir(TENGINE_DIR_CMOV);
	validate_dir(TENGINE_DIR_DATA);
	validate_dir(TENGINE_DIR_FILE);
	validate_dir(TENGINE_DIR_FONT);
	validate_dir(TENGINE_DIR_HELP);
	validate_dir(TENGINE_DIR_INFO);
	validate_dir(TENGINE_DIR_NOTE);
	validate_dir(TENGINE_DIR_PREF);
	validate_dir(TENGINE_DIR_RAW);
	validate_dir(TENGINE_DIR_SAVE);
	validate_dir(TENGINE_DIR_USER);
	validate_dir(TENGINE_DIR_XTRA);

	/* Build the filename */
	path_build(path, PATH_MAX, TENGINE_DIR_FONT, DEFAULT_FONT);
	...
	path_build(path, PATH_MAX, TENGINE_DIR_XTRA, "sound");

	/* Allocate the path */
	TENGINE_DIR_XTRA_SOUND = string_make(path);

	/* Validate the "sound" directory */
	validate_dir(TENGINE_DIR_XTRA_SOUND);

#endif


	/* Build the "help" path */
	path_build(path, PATH_MAX, TENGINE_DIR_XTRA, "help");

	/* Allocate the path */
	TENGINE_DIR_XTRA_HELP = string_make(path);

	/* Validate the "help" directory */
	/* validate_dir(TENGINE_DIR_XTRA_HELP); */
}
                       ...
                       LPSTR lpCmdLine, int nCmdShow)
{
	...
	/* Did the user double click on a team file? */
	check_for_module_file(lpCmdLine);
